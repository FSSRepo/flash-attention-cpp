...
typedef wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, __half, wmma::row_major> MatrixA;
typedef wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, __half, wmma::col_major> MatrixBT;
typedef wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, __half, wmma::row_major> MatrixB;
typedef wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float>                  Accum;
typedef wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, half>                  AccumH;
...
#pragma unroll
    ...
    wmma::fill_fragment(zr, 0.0);

    // zero out lo
    for (int j = 0; j < Q16; ++j) {
        for (int i = 0; i < D16; ++i) {
            wmma::fill_fragment(lo[j][i], 0.0);
        }
    }
...

    {
        ...
        for (int j = 0; j < Q16; ++j) {
            for (int i = 0; i < D16; ++i) {
                wmma::load_matrix_sync(mq[j][i], sq + 16*j*T + i*16, T);
            }
        }
        ...
        wmma::load_matrix_sync(mscale, ss, T);
...
#pragma unroll
                for (int cc = 0; cc < C16; ++cc) {
                    ...
                    for (int i = 0; i < D16; ++i) {
                        ...
                        wmma::load_matrix_sync(mk, pk + i*16, nb11/sizeof(half));

                        for (int j = 0; j < Q16; ++j) {
                            wmma::mma_sync(mqk[j], mq[j][i], mk, mqk[j]);
                        }
                    }
                    ...
                    for (int j = 0; j < Q16; ++j) {
                        ...
                        if (mp) {
                            wmma::load_matrix_sync(mm, mp + 16*j*(nb31/sizeof(half)) + ic + 16*cc, nb31/sizeof(half), wmma::mem_row_major);
                        }

                        // convert accumulator to matrix_a
                        wmma::store_matrix_sync(      ss + 16*j*T + 16*cc, mqk[j], T, wmma::mem_row_major);
                        wmma::load_matrix_sync (mqka, ss + 16*j*T + 16*cc, T);

                        wmma::mma_sync(mqk[j], mqka, mscale, mp ? mm : zr);
                        wmma::store_matrix_sync(ss + 16*j*T + 16*cc, mqk[j], T, wmma::mem_row_major);
                    }
                }
            }
            ...
            for (int j = 0; j < Q16; ++j) {
                ...
                wmma::load_matrix_sync(mm, ss + 16*j*T + C + 16*j, T);

                for (int i = 0; i < D16; ++i) {
                    // convert accumulator to matrix_b
                    wmma::store_matrix_sync(     ss + 16*j*T + C + 16*j, lo[j][i], T, wmma::mem_row_major);
                    wmma::load_matrix_sync (lob, ss + 16*j*T + C + 16*j, T);
                    wmma::mma_sync(lo[j][i], mm, lob, zr);

                    // if(warp_id == 1) {
                    //     wmma::store_matrix_sync(sq + 16*j*T + i*16, lo[j][i], T, wmma::mem_row_major);
                    // }
                }
            ...
            // restore zeros
            for (int j = 0; j < Q16; ++j) {
                wmma::store_matrix_sync(ss + 16*j*T + C + 16*j, zr, T, wmma::mem_row_major);
            }
            ...
            // O = O + (Q*K^T)*V
            {
                for (int cc = 0; cc < C16; ++cc) {
                    ...
                    MatrixB mv[D16];
                    for (int i = 0; i < D16; ++i) {
                        wmma::load_matrix_sync(mv[i], pv + i*16, nb21/sizeof(half));
                    }

                    for (int j = 0; j < Q16; ++j) {
                        MatrixA ms;
                        wmma::load_matrix_sync(ms, ss + 16*j*T + 16*cc, T);
                        for (int i = 0; i < D16; ++i) {
                            wmma::mma_sync(lo[j][i], ms, mv[i], lo[j][i]);
                        }
                    }
                }
            }
    ...
    for (int sg = 1; sg < num_warps; ++sg) {
        ...
        if (warp_id == 0) {
            ...
            for (int j = 0; j < Q16; ++j) {
                ...
                wmma::load_matrix_sync(ms0, ss + 16*j*T + C + 16*j,         T);
                wmma::load_matrix_sync(ms1, ss + 16*j*T + C + 16*j + sg*SH, T);

                for (int i = 0; i < D16; ++i) {
                    wmma::load_matrix_sync(t, sq + 16*j*T + i*16, T);
                    wmma::mma_sync(t2, ms1, t, zr);

                    // convert accumulator to matrix_b
                    wmma::store_matrix_sync(   sq + 16*j*T + i*16, lo[j][i], T, wmma::mem_row_major);
                    wmma::load_matrix_sync (t, sq + 16*j*T + i*16, T);

                    wmma::mma_sync(lo[j][i], ms0, t, t2);
                }
            }
        }
    }
